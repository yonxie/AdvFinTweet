...
class MessageAttention(torch.nn.Module):
    ...
    def __init__(self, input_size, dpt_in, msg_n_max=30):
        ...
        self.w = nn.Sequential(
            ...
            # nn.BatchNorm1d(msg_n_max, affine=False, track_running_stats=False),
            nn.Tanh(),
...
class TweetGRU(BaseModel):
    ...
    def __init__(self, ebd_size, fea_size, msg_hdn_size, embedding, device, config, syn_layer=None, max_n_msgs=30):
        super(TweetGRU, self).__init__(embedding=embedding, device=device, config=config, syn_layer=syn_layer)
        ...
        self.msg_GRU = nn.Sequential(
            nn.GRU(input_size=ebd_size, hidden_size=msg_hdn_size, 
                   num_layers=1, bidirectional=True, batch_first=True),
        )
        self.msg_attention = MessageAttention(msg_hdn_size, config['model']['dropout_ce'], msg_n_max=self.max_n_msgs)
        ...
        self.day_GRU = nn.GRU(input_size=self.feature_size+self.msg_hdn_size, num_layers=1,
    ...
    def normal_forward(self, inputs, **keywords):
        ...
        _, fea_hdn = self.day_GRU(fea_hdn)
...
class TweetGRUWrapper(BaseWrapper):
    def __init__(self, name, network, optimizer, votes, cfg):
        super(TweetGRUWrapper, self).__init__(name=name, network=network, optimizer=optimizer, votes=votes, cfg=cfg)
